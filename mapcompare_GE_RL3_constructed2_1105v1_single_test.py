...
def mapcompare_d500_first_update_new(fgt,fd,fr,county,radius):
    ...
    if not os.path.exists('../output2_GE_directly_add_RL3_constructed2_1105v1/'):
        os.makedirs('../output2_GE_directly_add_RL3_constructed2_1105v1/')
    df.to_csv('../output2_GE_directly_add_RL3_constructed2_1105v1/'+county+'_'+str(i)+'_test.csv', index=False)
...
def main():
    ...
    for county in ['130634']:#['411523', '610426','130631','130636', '130529', '341203', '130532', '130531', '130434']:#'130634',
    ...
    # ['130631', '131123', '131122', '141034', '130925', '131128', '341203', '131124', '130434', '522635']:#['610328', '130533', '130529', '130532', '130531'][:2]:#['130522']:#['610328', '130533', '130529', '130532', '130531', '130631', '131123', '131122', '141034', '130925', '131128', '341203', '131124', '130434', '522635']:#['130522']:#district_list:
        print(county)
        if os.path.exists('../output2_GE_directly_add_RL3_constructed2_1105v1/'+county+'_'+str(0.01)+'_test.csv'):
            continue
        ...
        fd = '../GraphSamplingToolkit-main_improve_GE_directly_add_RL3_constructed2_1105v1/'+county+'/xyx_0'+'/algorithm/'+county+'_pred'
        # fd = 'GraphSamplingToolkit-main_improve_GE/'+district+str(zoom)+'_'+str(k_size)+'/algorithm/xyx_'+str(radius)+'/'+district+'_'+str(year)
        
        fr = '../GraphSamplingToolkit-main_improve_GE_directly_add_RL3_constructed2_1105v1/'+county+'/xyx_0'+'/evals/'+county
        ...
        if not os.path.exists('../GraphSamplingToolkit-main_improve_GE/'+county+'/xyx_0/groundtruth/'+county+'_groundtruth_txt_edges_osm.txt'):
            continue
        if not os.path.exists('../GraphSamplingToolkit-main_improve_GE_directly_add_RL3_constructed2_1105v1/'+county+'/xyx_0/algorithm/'+county+'_pred_edges.txt'):
            continue
        if os.path.getsize('../GraphSamplingToolkit-main_improve_GE/'+county+'/xyx_0/groundtruth/'+county+'_groundtruth_txt_edges_osm.txt') == 0:
            continue
        if os.path.getsize('../GraphSamplingToolkit-main_improve_GE_directly_add_RL3_constructed2_1105v1/'+county+'/xyx_0/algorithm/'+county+'_pred_edges.txt') == 0:
            continue
        # mapcompare_d500_first_update_new(fgt,fd,fr,county,0)
        param_list.append([fgt,fd,fr,county,0])
        # mapcompare_d500_first_update_new(fgt,fd,fr,county,0)
    ...
    with Pool(processes=1) as pool:
        # 执行每个县和年份的图像处理任务
        tasks = [(k[0],k[1],k[2],k[3],k[4]) for k in param_list]
        results = pool.starmap(mapcompare_d500_first_update_new, tasks)
